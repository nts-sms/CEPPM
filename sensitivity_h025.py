...
HARVEST_RATE     = 0.25   # fixed at h_crit
INTERVENTION_YR  = 2
T_MAX            = 200
...
# ── Print 1D table ────────────────────────────────────────────────────────────
print("\n" + "="*72)
print(f"1D SENSITIVITY: varying delta  (r_stoat=0.60, h=0.25, intervention yr=2, t=200)")
print(f"Baseline: delta=0.35 → h_crit=0.25 (h=h_crit exactly)")
print("="*72)
...
for d, hc, xf, Kf, Sf in zip(DELTA_RANGE, hcrit_d, x_d, K_d, S_d):
    rel = "= h_crit" if abs(hc - 0.25) < 0.01 else ("> h_crit" if hc < 0.25 else "< h_crit")
    print(f"  {d:6.3f}   {hc:6.3f}   {rel:>12}   {xf*100:8.1f}%   {Kf:8.1f}   {Sf:8.3f}")

print("\n" + "="*72)
print(f"1D SENSITIVITY: varying r_stoat  (delta=0.35, h=0.25, intervention yr=2, t=200)")
print(f"Baseline: r_stoat=0.60 → h_crit=0.25 (h=h_crit exactly)")
print("="*72)
...
ax1.set_title('Open foraging vs δ\n(r_stoat=0.60 fixed, h=0.25)', fontsize=10)
...
ax3.axhline(0.25, color='orange', ls='--', lw=2, label='h=0.25 (our fixed harvest)')
...
ax3.set_title('h_crit vs δ\n(crossover shows where h=0.25 becomes sufficient)', fontsize=10)
...
ax6.axhline(0.25, color='orange', ls='--', lw=2, label='h=0.25 (our fixed harvest)')
...
ax6.set_title('h_crit vs r_stoat\n(crossover shows where h=0.25 flips)', fontsize=10)
...
r_hcrit_line = DELTA_RANGE + 0.25
mask = (r_hcrit_line >= RSTOAT_RANGE[0]) & (r_hcrit_line <= RSTOAT_RANGE[-1])
ax7.plot(DELTA_RANGE[mask], r_hcrit_line[mask], 'k-', lw=2.5, label='h_crit=0.25 line')
...
ax7.set_title('Open foraging % — 2D\n(black line = h_crit=0.25 boundary)', fontsize=10)
...
ax8.contour(D_mesh, R_mesh, K_grid, levels=[K0], colors='white', linewidths=1.5,
            linestyles='--')
ax8.plot(DELTA_RANGE[mask], r_hcrit_line[mask], 'k-', lw=2.5, label='h_crit=0.25 line')
...
regime = np.where(R_mesh - D_mesh > 0.25, 1.0, 0.0)   # 1=h sufficient, 0=not
regime[R_mesh <= D_mesh] = np.nan
im9 = ax9.contourf(D_mesh, R_mesh, regime, levels=[-0.5, 0.5, 1.5],
                   colors=['#d62728', '#2ca02c'], alpha=0.6)
ax9.plot(DELTA_RANGE[mask], r_hcrit_line[mask], 'k-', lw=3, label='h_crit=0.25 boundary')
ax9.plot(BASELINE_DELTA, BASELINE_RSTOAT, 'w*', ms=16, zorder=5, label='Baseline')
from matplotlib.patches import Patch
ax9.legend(handles=[
    Patch(facecolor='#d62728', alpha=0.6, label='h=0.25 insufficient (stoats win)'),
    Patch(facecolor='#2ca02c', alpha=0.6, label='h=0.25 sufficient (kiwi recover)'),
    plt.Line2D([0],[0], color='k', lw=2.5, label='h_crit=0.25 boundary'),
...
ax9.set_title('Regime map: h=0.25 sufficient?\n(green=kiwi recover, red=kiwi lost)', fontsize=10)
ax9.grid(True, alpha=0.2)

fig.suptitle(
    'Sensitivity at h=0.25 (critical harvest threshold), intervention yr=2, t=200\n'
    'Rows: (1) vary δ  (2) vary r_stoat  (3) joint 2D space',
    fontsize=12, fontweight='bold'
)

plt.savefig('/mnt/user-data/outputs/sensitivity_h025_fixed.png', dpi=150, bbox_inches='tight')
print("\nPlot saved.")
